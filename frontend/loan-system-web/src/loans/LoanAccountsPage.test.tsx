...
it('offers authorized pre-disbursement request and financing type change using ETag',async()=>{
  fetchMock.mockImplementation((url:string)=>url==='/api/v1/loans?'?json({items:[loan],pageNumber:1,pageSize:25,totalCount:1}):url===`/api/v1/loans/${loan.loanId}/financing-type`?json({...loan,financingType:'Purchase',eTag:'v2'}):url===`/api/v1/loans/${loan.loanId}`?json(loan):url===`/api/v1/loan-applications/${loan.sourceApplicationId}`?json(application):json(borrower));
  show(['borrowers.read','loanApplications.read','disbursements.create','loans.changeFinancingType']);await userEvent.click(await screen.findByTitle(loan.loanId));expect(await screen.findByRole('heading',{name:'Request Disbursement'})).toBeVisible();await userEvent.selectOptions(screen.getByRole('combobox',{name:'Change Financing Type'}),'Purchase');await userEvent.click(screen.getByRole('button',{name:'Confirm Change'}));await waitFor(()=>expect(fetchMock).toHaveBeenCalledWith(`/api/v1/loans/${loan.loanId}/financing-type`,expect.objectContaining({method:'POST',headers:expect.objectContaining({'If-Match':'"v"'})})));expect(screen.getByText('Purchase')).toBeVisible();
});

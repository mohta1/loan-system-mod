...
internal static class DocumentEndpoints
{
    ...
    static async Task<IResult> Metadata(Guid id, ClaimsPrincipal p, IDocumentRepository repo, IDocumentAccessAuthorizer auth, CancellationToken ct) { var d = await repo.FindAsync(id, ct); if (d is null || d.Status != DocumentStatus.Active) return Problem(404, "Document is unavailable", "documents.notFound"); if (!auth.CanAccess(d.UploaderId, User(p))) return Problem(403, "Document access is denied", "documents.forbidden"); return Results.Ok(DocumentService.Map(d)); }
    static async Task<IResult> Content(Guid id, ClaimsPrincipal p, IDocumentRepository repo, IDocumentAccessAuthorizer auth, DocumentService service, CancellationToken ct) { var d = await repo.FindAsync(id, ct); if (d is null || d.Status != DocumentStatus.Active) return Problem(404, "Document is unavailable", "documents.notFound"); if (!auth.CanAccess(d.UploaderId, User(p))) return Problem(403, "Document access is denied", "documents.forbidden"); var stream = await service.ContentAsync(d, ct); return stream is null ? Problem(404, "Document content is unavailable", "documents.contentMissing") : Results.File(stream, d.ContentType, SanitizeDownloadName(d.FileName), enableRangeProcessing: true); }
    static async Task<IResult> Delete(Guid id, ClaimsPrincipal p, IDocumentRepository repo, IDocumentAccessAuthorizer auth, DocumentService service, CancellationToken ct) { var d = await repo.FindAsync(id, ct); if (d is null || d.Status == DocumentStatus.Deleted) return Problem(404, "Document is unavailable", "documents.notFound"); if (!auth.CanAccess(d.UploaderId, User(p))) return Problem(403, "Document access is denied", "documents.forbidden"); try { await service.DeleteAsync(d, ct); return Results.NoContent(); } catch (IOException) { return Problem(503, "Document deletion is pending retry", "documents.storageUnavailable"); } }

...
public sealed class LoanProductService(ILoanProductStore store, IBusinessClock clock)
{
 ...
 public async Task<ProductVersionDto?> PublishAsync(Guid id,Guid versionId,CancellationToken ct){var v=await store.FindVersionAsync(id,versionId,ct);if(v is null)return null;if(await store.OverlapsAsync(id,v.Id,v.EffectiveFrom,v.EffectiveTo,ct))throw new EffectivePeriodConflictException();v.Publish(clock.UtcNow);await store.SaveAsync(ct);return Map(v);}

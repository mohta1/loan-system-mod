...
public sealed class LoanProductService(ILoanProductStore store, IBusinessClock clock)
{
    ...
    public async Task<ProductVersionDto?> CreateDraftAsync(Guid id, VersionInput input, CancellationToken ct)
    {
        var p = await store.FindAsync(id, ct);
        if (p is null) return null;
        try
        {
            var number = await store.NextVersionNumberAsync(id, ct);
            var v = p.CreateDraftVersion(number, input.MaximumAmount, input.Currency, input.DeductionPercentage, input.FinancingTypes, Eligibility(input.EligibilityConfiguration), input.EffectiveFrom, input.EffectiveTo, clock.UtcNow);
    ...
    public async Task<ProductVersionDto?> PublishAsync(Guid id, Guid versionId, byte[] expected, CancellationToken ct)
    {
        var v = await store.FindVersionAsync(id, versionId, ct);
        if (v is null) return null;
        try
        {
            await store.EnsureVersionAsync(v, expected, ct);
            await store.LockProductAsync(id, ct);
            if (await store.OverlapsAsync(id, v.Id, v.EffectiveFrom, v.EffectiveTo, ct)) throw new EffectivePeriodConflictException();
            v.Publish(clock.UtcNow);
            store.Expect(v, expected);
            await store.SaveAsync(ct);
            return Map(v);
        }
        catch

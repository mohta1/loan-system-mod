...
public sealed class PropertyInspection
{
    ...
    public PropertyInspectionRecorded Complete(Guid actor, DateTimeOffset? now = null) { EnsureEditable(); if (Status != PropertyInspectionStatus.Draft) throw new InspectionStateException(); ValidateComplete(); var at = now ?? DateTimeOffset.UtcNow; Status = PropertyInspectionStatus.Recorded; UpdatedAtUtc = at; var e = new PropertyInspectionRecorded(Id, LoanApplicationId, actor, at); _domainEvents.Add(e); return e; }
    public PropertyInspectionApproved Approve(Guid actor, DateTimeOffset? now = null) { EnsureRecorded(); var at = now ?? DateTimeOffset.UtcNow; Status = PropertyInspectionStatus.Approved; UpdatedAtUtc = at; FinalizedAtUtc = at; var e = new PropertyInspectionApproved(Id, LoanApplicationId, actor, at); _domainEvents.Add(e); return e; }
    public PropertyInspectionRejected Reject(Guid actor, string? reason, DateTimeOffset? now = null) { EnsureRecorded(); if (string.IsNullOrWhiteSpace(reason)) throw new InspectionRejectionReasonRequiredException(); var at = now ?? DateTimeOffset.UtcNow; Status = PropertyInspectionStatus.Rejected; UpdatedAtUtc = at; FinalizedAtUtc = at; var e = new PropertyInspectionRejected(Id, LoanApplicationId, actor, at, reason.Trim()); _domainEvents.Add(e); return e; }

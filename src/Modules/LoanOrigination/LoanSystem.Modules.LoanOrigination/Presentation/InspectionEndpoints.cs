...
internal static class InspectionEndpoints
{
    ...
    private static async Task<IResult> AttachDocument(Guid applicationId, AttachRequest input, HttpContext c, LoanApplicationService s, CancellationToken ct) => await AppMutation(c, v => { if (!Enum.TryParse<LoanApplicationDocumentType>(input.DocumentType, true, out var type)) throw new InvalidDocumentTypeException(); return s.AttachDocumentAsync(applicationId, input.DocumentId, type, Actor(c), v, ct); });

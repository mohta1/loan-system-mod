...
public sealed class UserAdministration
{
    ...
    public async Task<UserDto?> AssignRolesAsync(Guid id, IReadOnlyCollection<Guid>? roles, byte[] version, CancellationToken ct) { if (roles is null) throw new ArgumentException("Roles are required."); var u = await store.FindAsync(id, ct); if (u is null) return null; await store.EnsureAdministratorContinuityAsync(u, u.IsActive, roles, ct); store.SetExpectedVersion(u, version); u.Touch(DateTimeOffset.UtcNow); await store.ReplaceRolesAsync(u, roles, ct); await store.SaveAsync(ct); return Map(u); }

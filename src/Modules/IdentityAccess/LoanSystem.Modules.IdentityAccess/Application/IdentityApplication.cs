...
public interface IIdentityStore
{
    ...
    Task AddAsync(User user, CancellationToken ct); Task ReplaceRolesAsync(User user, IReadOnlyCollection<Guid> roleIds, CancellationToken ct); void SetExpectedVersion(User user, byte[] version); Task SaveAsync(CancellationToken ct);
}
...
public sealed class UserAdministration
{
    ...
    public async Task<UserDto?> SetActiveAsync(Guid id, bool active, byte[] version, CancellationToken ct) { var u = await store.FindAsync(id, ct); if (u is null) return null; store.SetExpectedVersion(u, version); u.SetActive(active, DateTimeOffset.UtcNow); await store.SaveAsync(ct); return Map(u); }
    ...
    public async Task<UserDto?> AssignRolesAsync(Guid id, IReadOnlyCollection<Guid>? roles, byte[] version, CancellationToken ct) { if (roles is null) throw new ArgumentException("Roles are required."); var u = await store.FindAsync(id, ct); if (u is null) return null; store.SetExpectedVersion(u, version); u.Touch(DateTimeOffset.UtcNow); await store.ReplaceRolesAsync(u, roles, ct); await store.SaveAsync(ct); return Map(u); }

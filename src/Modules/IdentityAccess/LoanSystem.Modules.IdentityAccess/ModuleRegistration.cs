...
public static class ModuleRegistration
{
    public static IServiceCollection AddIdentityAccessModule(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddHttpContextAccessor(); services.AddScoped<IIdentityStore>(x => x.GetRequiredService<IdentityAccessDbContext>()); services.AddScoped<IPasswordService, PasswordService>(); services.AddScoped<PasswordService>(); services.AddScoped<IIdentityProvider, LocalIdentityProvider>(); services.AddScoped<ICurrentUser, CurrentUser>(); services.AddScoped<IPermissionChecker, PermissionChecker>(); services.AddScoped<UserAdministration>(); services.AddScoped<IAuthorizationHandler, PermissionHandler>();
        ...
        services.AddAuthorizationBuilder().AddPolicy(IdentityPermissions.ManageUsers, policy => policy.AddRequirements(new PermissionRequirement(IdentityPermissions.ManageUsers)));
        return services;
    }

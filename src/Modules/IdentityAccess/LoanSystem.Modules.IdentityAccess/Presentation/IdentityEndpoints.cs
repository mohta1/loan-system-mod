...
internal static class IdentityEndpoints
{
    ...
    private static async Task<IResult> Active(Guid id, bool active, HttpRequest request, UserAdministration app, CancellationToken ct) { if (!TryVersion(request, out var version)) return Precondition(); try { var u = await app.SetActiveAsync(id, active, version, ct); return u is null ? Results.NotFound() : WithEtag(u); } catch (Microsoft.EntityFrameworkCore.DbUpdateConcurrencyException) { return Precondition(); } }
    private static async Task<IResult> Roles(Guid id, AssignRolesRequest r, HttpRequest request, UserAdministration app, CancellationToken ct) { if (!TryVersion(request, out var version)) return Precondition(); try { var u = await app.AssignRolesAsync(id, r.RoleIds, version, ct); return u is null ? Results.NotFound() : WithEtag(u); } catch (ArgumentException ex) { return Problem(400, ex.Message == "Roles are required." ? "Roles are required" : "Unknown role", ex.Message == "Roles are required." ? "identity.validation" : "identity.unknownRole"); } catch (Microsoft.EntityFrameworkCore.DbUpdateConcurrencyException) { return Precondition(); } }

# TASK-04 — Borrower Excel Import

    ## Objective

    Add validated, auditable borrower Excel import without bypassing Borrower domain rules. Users must preview errors before executing the import.
...
- `docs/loan-system-persistence-design.md`
    ...
    - Implement borrower import batch/row application workflow.
- Parse configured Excel template safely and normalize rows.
- Implement validate/preview endpoint and execute endpoint.
- Ensure execution reuses Borrower domain/application rules rather than raw bulk table writes.
- Make execution idempotent by batch identifier.
- Return row-level success/error codes.

    ## Frontend Scope

    - Create Borrower Import page with upload, validation summary, row error preview, and explicit execute action.
- Show final imported/failed counts and downloadable/visible error information.

    ## Persistence / Infrastructure Scope

    - Create/import batch and row persistence tables/migration.
- Reference source DocumentId when appropriate.

    ## Required Tests

    - Parser/unit tests for valid/invalid rows and duplicates.
- Integration tests for validate-without-write, execute, idempotent re-execution, partial invalid rows.
- Frontend tests for preview and execute states.
- E2E smoke with a small fixture workbook.

    ## Acceptance Criteria

    - [ ] Validation does not mutate borrower data.
- [ ] Execute imports only valid rows according to documented policy.
- [ ] Re-executing same batch does not duplicate borrowers.
- [ ] Failures are visible per row.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- This slice implements the source's indirect Excel borrower-data path for non-MOD organizations. Real HR direct integration remains deferred behind the borrower-source abstraction.
- Import errors and result screens must be available in English and Arabic.
...
- Upload a valid borrower Excel file and verify preview occurs before any import.
- Upload a file containing invalid/duplicate rows and verify row-level errors are shown.
- Execute a validated batch and verify successful borrowers appear in Borrowers.
- Retry the same batch and verify borrowers are not duplicated.
...
```bash
git switch -c task/04-borrower-import
```
...
git status
git add .
git commit -m "feat: complete TASK-04 borrower-import"
git push -u origin task/04-borrower-import
```

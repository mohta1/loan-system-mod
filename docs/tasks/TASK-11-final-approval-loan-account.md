# TASK-11 — Final Application Approval and Loan Account Opening

    ## Objective

    Complete Origination by final-approving a fully ready application, publishing a reliable integration event, and opening exactly one LoanAccount through Outbox/Inbox processing.
...
- `docs/loan-system-persistence-design.md`
    ...
    - Implement final approval domain behavior and all prerequisites.
- Publish `LoanApplicationApprovedV1` through transactional Outbox.
- Implement in-process Outbox dispatcher and LoanAccounts Inbox consumer if not already fully implemented.
- Implement LoanAccount opening idempotent by SourceApplicationId.
- Implement get/search loan account read endpoints.
- Expose processing state/error observability needed to diagnose delayed consumer handling.

    ## Frontend Scope

    - Add Final Approval action to ready application review.
- Create Loans list/detail page showing approved amount, reserved, disbursed, repaid, balance, and status.
- After approval, UI handles eventual creation of LoanAccount without assuming same-transaction availability.

    ## Persistence / Infrastructure Scope

    - Create LoanAccounts tables/migration, Outbox/Inbox tables for involved modules, required unique SourceApplicationId constraint.

    ## Required Tests

    - Domain tests for final approval prerequisites.
- Outbox persistence integration test verifies application state + event commit atomically.
- Inbox/idempotency tests verify duplicate `LoanApplicationApprovedV1` creates one LoanAccount.
- Failure/retry test proves consumer can recover.
- Frontend tests for final approval and pending-to-loan navigation.
- E2E smoke: ready application → approve → LoanAccount appears.

    ## Acceptance Criteria

    - [ ] Application approval and LoanAccount creation are separate transactions.
- [ ] Exactly one LoanAccount exists per approved application.
- [ ] Event loss/duplicate delivery is safely handled.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Add the MVP simple LoanAccount cancellation path: an opened loan may be cancelled only when `TotalDisbursed == 0` and there is no unresolved active financial obligation/reservation. Cancellation must be audited.
- Final approval must remain separate from LoanAccount creation via the documented integration event; no cross-module transaction shortcut.
...
- Final-approve an application with all prerequisites and verify a LoanAccount appears after event processing.
- Try final approval with a missing prerequisite and verify it is rejected.
- Refresh/poll the UI during the short eventual-consistency window and verify the UI handles pending LoanAccount creation.
- Cancel an approved/opened loan before any disbursement where allowed and verify it becomes cancelled/audited.
- Verify the same approved application never creates a second LoanAccount.
...
```bash
git switch -c task/11-final-approval-loan-account
```
...
git status
git add .
git commit -m "feat: complete TASK-11 final-approval-loan-account"
git push -u origin task/11-final-approval-loan-account
```

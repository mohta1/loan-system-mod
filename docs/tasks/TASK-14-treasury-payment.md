# TASK-14 — Treasury Maker-Checker and Fake Payment Execution

    ## Objective

    Implement Treasury input, audit, final approval, payment execution, retry-safe fake gateway integration, and completion feedback to both LoanAccount and Disbursement.
...
- `docs/loan-system-docker-runtime.md`
    ...
    - Implement TreasuryPayment aggregate creation idempotent by DisbursementId.
- Implement Input → Audit → Approve maker-checker workflow.
- Enforce configurable separation-of-duty rules server-side.
- Implement `IPaymentGateway` and deterministic FakePaymentGateway for MVP/test modes.
- Implement execute/retry with HTTP and gateway idempotency.
- Publish PaymentCompleted/PaymentFailed.
- Consume completion in LoanAccounts to convert reservation to disbursed amount.
- Consume completion/failure in Disbursements to update status.

    ## Frontend Scope

    - Create Treasury work queues for Input, Audit, Approval, and Failed payments.
- Create payment detail showing approvals/attempts/result.
- Create execute/retry controls only for permitted state/permission.
- Update Loan/Disbursement screens after payment completion.

    ## Persistence / Infrastructure Scope

    - Create TreasuryPayment and PaymentAttempt migrations and indexes.
- Ensure unique DisbursementId logical payment constraint.

    ## Required Tests

    - Domain tests for maker-checker transitions and separation of duties.
- Integration tests for duplicate ReadyForTreasury event, execute idempotency, gateway success/failure/retry.
- Critical duplicate-success callback/event test proves LoanAccount is incremented once.
- Frontend tests for queues and payment actions.
- E2E smoke: approved disbursement → treasury input → audit → approve → fake payment success → loan balance updated.

    ## Acceptance Criteria

    - [ ] One disbursement cannot be paid twice.
- [ ] Payment success updates LoanAccount and Disbursement through reliable events.
- [ ] Failed retry does not create a second logical payment.
- [ ] Maker/checker constraints are server enforced.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Add pre-payment return behavior required by the source: Treasury Auditor → Input, Treasury Approver → Audit, and Treasury Input → administrative side when correction is required before payment execution. Every return requires reason/history.
- Real B2B integration remains deferred; use `IPaymentGateway` + fake adapter.
- Post-payment correction/recovery is explicitly deferred by approved MVP scope; do not invent accounting reversal semantics.
...
- Process a ReadyForTreasury item through Input → Auditor → Approver and execute the fake payment.
- Verify maker/checker separation prevents the same prohibited actor combination where configured.
- Return a pre-payment transaction to the prior Treasury/admin stage with a reason and verify history.
- Exercise fake payment failure and retry, then verify only one logical payment exists.
- Exercise success and verify Disbursement completes and LoanAccount TotalDisbursed changes exactly once.
- Verify no post-payment correction/reversal UI is exposed because that full-scope feature is deferred.
...
```bash
git switch -c task/14-treasury-payment
```
...
git status
git add .
git commit -m "feat: complete TASK-14 treasury-payment"
git push -u origin task/14-treasury-payment
```

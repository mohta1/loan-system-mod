# TASK-13 — Administrative Disbursement Approvals

    ## Objective

    Implement Technical Affairs, Accounting, and Higher Officer approval/rejection steps and release reserved capacity whenever a request is rejected/cancelled.
...
- `docs/loan-system-module-contracts.md`
    ...
    - Implement technical approve/reject.
- Implement accounting approve/reject requiring technical approval.
- Implement final/higher approve/reject requiring accounting approval.
- Publish `DisbursementReadyForTreasuryV1` on final approval.
- Publish capacity release request on rejection/cancellation and handle idempotently in LoanAccounts.
- Persist actor/timestamp/comment/reason for each decision.

    ## Frontend Scope

    - Create role-specific Disbursement work queues.
- Create decision controls and approval history.
- Show reservation release/result after rejection.

    ## Persistence / Infrastructure Scope

    - Add migration only if approval/history persistence requires it.

    ## Required Tests

    - Domain transition tests for all approval levels.
- Integration tests for skip-step attempts, permission denial, rejection release, duplicate release, concurrency.
- Frontend tests for each approval queue/action.
- E2E smoke: requested disbursement → technical → accounting → final approval.

    ## Acceptance Criteria

    - [ ] Approval order cannot be skipped.
- [ ] Rejected/cancelled request releases capacity exactly once.
- [ ] Final approval publishes ReadyForTreasury reliably.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Add `Return for Correction` transitions required by the source: Technical → request/correction state, Accounting → Technical, Higher Officer → Accounting. Every return requires a reason and full transition history/audit event.
- Return is non-terminal and distinct from Reject.
- Rejection/cancellation must release reserved capacity idempotently.
...
- Approve a Requested disbursement through Technical → Accounting → Higher Officer in order.
- Try to skip an approval stage and verify it is blocked.
- Use Return for Correction at Technical/Accounting/Higher stage and verify it goes to the documented prior stage with reason/history.
- Reject/cancel a request and verify reserved capacity is released.
- Verify only fully approved requests become ReadyForTreasury.
...
```bash
git switch -c task/13-disbursement-approvals
```
...
git status
git add .
git commit -m "feat: complete TASK-13 disbursement-approvals"
git push -u origin task/13-disbursement-approvals
```

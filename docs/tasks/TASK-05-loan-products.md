# TASK-05 — Loan Products and Versioning

    ## Objective

    Implement configurable versioned loan products so business rules are data/configuration driven and historical applications can snapshot the exact published version they used.
...
- `docs/loan-system-persistence-design.md`
    ...
    - Implement LoanProduct aggregate and immutable published LoanProductVersion behavior.
- Implement create product, draft version, edit draft, publish, activate/deactivate, and list available versions.
- Implement maximum amount, deduction percentage, financing types, effective period, and eligibility configuration contract.
- Expose `ILoanProductsModule.GetVersionAsync` through Contracts.

    ## Frontend Scope

    - Create Loan Products list/detail/configuration pages.
- Create draft-version form and publish action.
- Clearly distinguish Draft/Published and Active/Inactive states.
- Prevent editing published versions in UI while backend remains authoritative.

    ## Persistence / Infrastructure Scope

    - Create loan product/version/financing-type migration and indexes.
- Persist variable eligibility configuration using the agreed JSON representation.

    ## Required Tests

    - Domain tests for published immutability, valid effective period, positive limits, status transitions.
- Integration tests for create/edit/publish/activate/deactivate and unavailable product queries.
- Frontend tests for draft/edit/publish state.
- E2E smoke creates and publishes a product version.

    ## Acceptance Criteria

    - [ ] Published version cannot be mutated.
- [ ] Available-product endpoint returns only eligible published/active versions according to current policy.
- [ ] React can configure and publish a product end-to-end.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Product/rule configuration must be capable of representing source requirements for financing type, deduction percentage, effective period, due-date/term, required nationality, maximum application count, and rank/grade-based amount configuration.
- Include the source financing types `Purchase Existing House` and `Build New House` as configurable data/demo seed values, not hard-coded branching logic. New financing types must remain addable.
- Do not hard-code source examples such as 30%/40% deduction values into domain/controller/UI code.
...
- Create a loan product and draft version with financing types and configurable rules.
- Configure purchase-existing-house and build-new-house financing types and verify both appear.
- Publish a version and verify published fields cannot be edited.
- Deactivate the product and verify it is no longer offered for new applications.
- Verify rule fields such as nationality/application-count/rank-grade amount are visible as configuration, not hard-coded UI text.
...
```bash
git switch -c task/05-loan-products
```
...
git status
git add .
git commit -m "feat: complete TASK-05 loan-products"
git push -u origin task/05-loan-products
```

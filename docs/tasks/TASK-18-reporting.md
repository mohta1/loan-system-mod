# TASK-18 — Operational Reporting

    ## Objective

    Implement the MVP read/reporting projections and React reporting screens without coupling transactional modules to Reporting.
...
- `docs/loan-system-persistence-design.md`
    ...
    - Implement reporting projections for applications, active loans, disbursements, repayments, and outstanding balances.
- Update projections from integration events or explicit read feeds according to the established model.
- Implement paginated/filterable report endpoints.
- Ensure Reporting never writes transactional business tables.

    ## Frontend Scope

    - Create Reports section/screens for the five MVP reports.
- Provide filters and consistent loading/error/empty states.
- Link report rows to source detail pages where authorized.

    ## Persistence / Infrastructure Scope

    - Create reporting projection tables/migrations and indexes as required.

    ## Required Tests

    - Projection update integration tests.
- Idempotent projection-event consumption tests.
- Report query/filter tests.
- Frontend report rendering/filter tests.
- E2E smoke verifies a prior business transaction appears in relevant report.

    ## Acceptance Criteria

    - [ ] Reports are read-only projections.
- [ ] Transactional modules do not depend on Reporting.
- [ ] Projection retries/duplicates are safe.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Implement approved operational MVP reports only. Full Power BI integration and arbitrary end-user smart-report authoring remain explicitly deferred, while the reporting boundary must not block them later.
...
- Open each MVP report: applications, active loans, disbursements, repayments, outstanding balances.
- Filter report data and navigate from an authorized row to the source detail where supported.
- Perform a business transaction and verify its reporting projection updates.
- Verify reporting screens are read-only and contain no transaction-changing actions.
...
```bash
git switch -c task/18-reporting
```
...
git status
git add .
git commit -m "feat: complete TASK-18 reporting"
git push -u origin task/18-reporting
```

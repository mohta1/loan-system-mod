# TASK-17 — Immutable Audit Trail

    ## Objective

    Implement a reliable, queryable audit trail from business/integration events without making Audit a source of transactional truth.
...
- `docs/loan-system-authorization-matrix.md`
    ...
    - Implement append-only AuditEntry persistence.
- Consume critical application, approval, disbursement, treasury, repayment, user/configuration events.
- Persist Actor, Action, Entity, Timestamp, state metadata, reason, CorrelationId, EventId.
- Implement authorized audit-search endpoint.
- Ensure duplicate event delivery does not duplicate audit entry.

    ## Frontend Scope

    - Create Audit Search page with filters for entity, actor, action, date, correlation ID.
- Link relevant business details to audit history where practical.

    ## Persistence / Infrastructure Scope

    - Create audit_entries migration and indexes.

    ## Required Tests

    - Integration tests for representative events from Origination, Disbursement, Treasury, Repayment.
- Duplicate-event idempotency test.
- Permission/search filter tests.
- Frontend search/filter tests.
- E2E smoke performs a business action and finds its audit entry.

    ## Acceptance Criteria

    - [ ] Audit is append-only.
- [ ] Business modules never query Audit to validate domain behavior.
- [ ] Critical actions are traceable by CorrelationId.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Ensure scope-relevant `Return for Correction`, cancellation, disbursement, payment and repayment actions are auditable with actor/reason/from/to/correlation metadata.
...
- Perform a business approval/payment/repayment action, open Audit, and find the event by entity/correlation ID.
- Filter audit entries by actor, action and date.
- Verify return-for-correction and cancellation actions include reason and state transition.
- Use a user without audit permission and verify access is denied.
...
```bash
git switch -c task/17-audit-trail
```
...
git status
git add .
git commit -m "feat: complete TASK-17 audit-trail"
git push -u origin task/17-audit-trail
```

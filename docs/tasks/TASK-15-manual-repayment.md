# TASK-15 — Manual Repayment

    ## Objective

    Implement manually recorded repayments end-to-end and apply accepted repayment exactly once to the LoanAccount outstanding balance.
...
- `docs/loan-system-persistence-design.md`
    ...
    - Implement Repayment aggregate for Manual source.
- Validate amount, loan acceptability, duplicate external reference, receipt requirements, and MVP overpayment rule.
- Implement manual repayment API with Idempotency-Key.
- Publish `RepaymentPostedV1` reliably.
- Consume in LoanAccounts idempotently and update totals/status.

    ## Frontend Scope

    - Create Repayments list.
- Create Record Manual Repayment form from Loan detail.
- Attach receipt using Documents component.
- Show repayment history and updated outstanding balance.

    ## Persistence / Infrastructure Scope

    - Create repayments migration and required indexes/unique constraints.
- Add Outbox/Inbox tables if not already present for Repayments/LoanAccounts.

    ## Required Tests

    - Domain tests for positive amount, duplicate/reference rules, overpayment policy.
- Integration tests for HTTP idempotency and duplicate event application.
- Loan balance test verifies repayment applied once.
- Frontend tests for form/errors/history.
- E2E smoke: paid loan → record manual repayment → outstanding balance decreases.

    ## Acceptance Criteria

    - [ ] Repayment cannot make balance negative under MVP policy.
- [ ] Duplicate retry/reference/event cannot double-apply payment.
- [ ] Receipt metadata is linked by DocumentId.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Manual repayment represents the source flow where the borrower deposits money through the bank and provides a receipt to update the loan. Partial and full repayment must be supported subject to MVP overpayment rules.
...
- Record a valid manual partial repayment with receipt and verify outstanding balance decreases.
- Attempt a repayment larger than the remaining balance and verify MVP overpayment policy rejects it.
- Retry/double-submit the same repayment/reference and verify it is not applied twice.
- Open repayment history and verify the posted transaction is immutable.
...
```bash
git switch -c task/15-manual-repayment
```
...
git status
git add .
git commit -m "feat: complete TASK-15 manual-repayment"
git push -u origin task/15-manual-repayment
```

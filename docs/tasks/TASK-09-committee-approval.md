# TASK-09 — Loan Committee Approval Workflow

    ## Objective

    Implement Committee approval/rejection after Unit approval, maintaining strict state order and separation of permission.
...
- `docs/loan-system-api-boundary.md`
    ...
    - Implement Committee approve/reject behaviors and handlers.
- Require prior Unit approval.
- Persist actor/timestamp/reason/comment.
- Enforce `loanApplications.committeeApprove` server-side.

    ## Frontend Scope

    - Create Committee Review queue for UnitApproved applications.
- Create decision UI and show prior Unit decision context.

    ## Persistence / Infrastructure Scope

    - No persistence changes unless decision storage requires it.

    ## Required Tests

    - Domain tests for Committee transition prerequisites.
- Integration tests for approve/reject, direct skip attempt, permission denial, stale update.
- Frontend tests for queue and decision UI.
- E2E smoke: unit-approved application → committee approval.

    ## Acceptance Criteria

    - [ ] Committee cannot approve before Unit.
- [ ] Committee rejection terminates normal approval flow.
- [ ] Frontend and API present consistent state/history.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Preserve source Committee queue semantics. Advanced role-sequence prioritization and exception-case nomination are explicitly deferred until rules are provided; do not invent them.
...
- Login as Committee user and verify UnitApproved applications appear in the Committee queue.
- Approve an application and verify CommitteeApproved state/history.
- Reject an application with a reason and verify it cannot continue normally.
- Try Committee approval before Unit approval and verify it is blocked.
...
```bash
git switch -c task/09-committee-approval
```
...
git status
git add .
git commit -m "feat: complete TASK-09 committee-approval"
git push -u origin task/09-committee-approval
```

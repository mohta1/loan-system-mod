# TASK-06 — Loan Application Draft

    ## Objective

    Create the Loan Origination draft slice. A loan officer selects an active borrower and published product version, and the system stores immutable borrower/product snapshots with the draft application.
...
- `docs/loan-system-api-boundary.md`
    ...
    - Implement LoanApplication aggregate creation/draft editing.
- Use `IBorrowersModule` and `ILoanProductsModule` synchronous read contracts.
- Persist borrower and product snapshots at the appropriate creation/decision point defined by the design.
- Implement create/update/get/search application endpoints for Draft state.
- Prevent direct cross-module persistence access.

    ## Frontend Scope

    - Create Loan Applications list page.
- Create New Application wizard/form selecting borrower and available loan product version.
- Create Draft detail/edit page showing captured product/borrower information.

    ## Persistence / Infrastructure Scope

    - Create initial `loan_origination.loan_applications` migration including snapshots, status, rowversion, and indexes.

    ## Required Tests

    - Domain tests for draft rules and requested amount validation.
- Module-contract tests verify Borrowers/LoanProducts are accessed only through contracts.
- Integration tests for create/update/get/search and inactive borrower/unavailable product failures.
- Frontend tests for draft form and selection flow.
- E2E smoke creates a Draft application.

    ## Acceptance Criteria

    - [ ] Application draft works end-to-end.
- [ ] No cross-module DbContext/implementation reference is introduced.
- [ ] Historical snapshot exists in application persistence.
- [ ] Draft concurrency is enforced.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Application creation must select one of the product's allowed financing types and persist the relevant product/borrower snapshot.
- Advanced post-approval financing-type switching is handled later by the Scope Alignment rule before disbursement; do not create an unsafe generic status/data-edit endpoint here.
...
- Create a Draft application by selecting an active borrower and published product version.
- Verify borrower/product snapshot information appears on the application detail.
- Edit a Draft application and verify changes persist.
- Try to create an application for an inactive borrower or unavailable product and verify it is blocked.
...
```bash
git switch -c task/06-loan-application-draft
```
...
git status
git add .
git commit -m "feat: complete TASK-06 loan-application-draft"
git push -u origin task/06-loan-application-draft
```

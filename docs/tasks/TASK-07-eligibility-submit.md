# TASK-07 — Eligibility Evaluation and Application Submission

    ## Objective

    Implement the first real loan-origination business decision: evaluate eligibility using the captured/published product configuration, persist the decision, and allow only eligible complete drafts to be submitted.
...
- `docs/loan-system-application-use-cases.md`
    ...
    - Implement explicit eligibility policy/evaluator with rule results and reason codes.
- Implement `EvaluateEligibility` and `SubmitLoanApplication` use cases.
- Persist EligibilityDecision/snapshot and applied product version.
- Enforce submit prerequisites in domain logic.
- Publish local/domain event `LoanApplicationSubmitted` as appropriate.
- Return stable business error codes.

    ## Frontend Scope

    - Add eligibility evaluation panel to application detail.
- Render rule results/reasons in user-readable form.
- Enable Submit only as UX guidance when eligible/complete; backend must enforce independently.
- Show Submitted status/history.

    ## Persistence / Infrastructure Scope

    - Add migration only if additional eligibility/history columns/tables are required.

    ## Required Tests

    - High-coverage domain tests for each implemented eligibility rule and edge condition.
- Submission negative tests: ineligible, incomplete, wrong state, stale version.
- Integration tests for evaluate/submit permissions and status transition.
- Frontend tests for eligible/ineligible rendering and submit flow.
- E2E smoke evaluates and submits an application.

    ## Acceptance Criteria

    - [ ] Eligibility decision is reproducible from persisted snapshot/rule version.
- [ ] Ineligible application cannot be submitted through direct API call.
- [ ] Submitted application cannot be freely edited as Draft.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Eligibility/amount calculation must demonstrate configurable support for: required nationality (including Omani when configured), maximum application count (including one application when configured), and rank/grade-based permitted/proposed amount.
- Rule outcomes must be persisted/explainable with stable reason codes. Values remain configuration/data-driven rather than hard-coded.
...
- Evaluate an application that satisfies configured rules and verify Eligible plus amount/rule results.
- Evaluate a borrower that violates required nationality and verify the rule reason is shown.
- Verify the configured single-application rule blocks a second conflicting application.
- Verify rank/grade-based amount calculation produces the configured permitted/proposed amount.
- Submit an eligible complete application and verify the state becomes Submitted.
- Try to submit an ineligible/incomplete application and verify it is rejected.
...
```bash
git switch -c task/07-eligibility-submit
```
...
git status
git add .
git commit -m "feat: complete TASK-07 eligibility-submit"
git push -u origin task/07-eligibility-submit
```

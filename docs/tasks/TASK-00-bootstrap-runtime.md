# TASK-00 — Bootstrap Solution, Frontend, Tests, and Docker

    ## Objective

    Create the runnable project skeleton that every later vertical slice will build on. At the end, an empty-but-real React application and ASP.NET Core API must start against SQL Server, the test projects must run, architecture rules must be executable, and Docker Compose must provide a reproducible local runtime.
...
- `docs/loan-system-solution-architecture.md`
- `docs/loan-system-docker-runtime.md`
- `docs/loan-system-testing-strategy.md`
- `docs/loan-system-architecture-decisions.md`

    ## Backend Scope

    - Create `LoanSystem.sln`, `LoanSystem.Api`, `LoanSystem.Contracts`, BuildingBlocks projects, and one implementation project for each planned module.
- Create internal module folder/namespace structure without implementing business features yet.
- Configure nullable reference types, warnings/analyzers, consistent build settings, ProblemDetails, correlation ID middleware, OpenAPI, liveness/readiness health endpoints, and module registration pattern.
- Create initial architecture-test rules for forbidden dependency directions.
- Create GitHub Actions Pull Request CI (`.github/workflows/ci.yml`) and make its checks the baseline required before any later feature PR may merge.
- Create xUnit-based test projects and SQL Server integration-test harness using Testcontainers.

    ## Frontend Scope

    - Create React + TypeScript + Vite application.
- Configure React Router, TanStack Query provider, error boundary/application shell, API base configuration, TypeScript strict mode, linting and test runner.
- Configure frontend i18n with English and Arabic resources plus document direction switching for LTR/RTL.
- Create a minimal home/system-status screen that calls API health/readiness or a simple `/api/v1/system/info` endpoint so frontend/backend connectivity is proven.
- Configure React Testing Library and browser/E2E test baseline.

    ## Persistence / Infrastructure Scope

    - Create `compose.yaml` with SQL Server, API, and web services.
- Create `.env.example` with no real secrets.
- Create API and frontend Dockerfiles.
- Create named SQL Server and file-storage volumes.
- Create platform schema/migration bootstrap if required by migration runner design.
- Document local commands in repository README.
- Configure GitHub `main` branch protection/ruleset to require Pull Requests and CI status checks where repository permissions allow; otherwise provide the user exact settings to enable before merging feature work.

    ## Required Tests

    - Architecture test proves module implementations cannot reference other module implementations.
- API integration test proves application boots against real SQL Server Testcontainer.
- Health/readiness endpoint tests.
- React application-shell/component tests.
- One E2E/smoke test proves browser → React → API connectivity.

    ## Acceptance Criteria

    - [ ] `docker compose up --build` starts SQL Server, API, and React without manual code edits.
- [ ] `dotnet test` passes from repository root.
- [ ] Frontend typecheck/lint/test/build pass.
- [ ] Architecture tests are active, not placeholders.
- [ ] README contains reproducible local startup instructions.

## Scope / Documentation Alignment Requirements

- Create `.github/workflows/ci.yml` as part of the initial bootstrap, before any business-feature task begins. The Pull Request workflow must run backend restore/build/tests/coverage/architecture tests and frontend install/typecheck/lint/tests/coverage/build. Include integration tests using the repository-standard SQL Server Testcontainers setup and a Docker build/smoke job once the bootstrap images exist.
- Configure or document a GitHub `main` branch ruleset/branch protection requiring Pull Requests and the CI status checks before merge. If repository permissions do not allow Codex to configure this automatically, it must report the exact required GitHub settings and mark the protection step as requiring user action.
- Establish React internationalization from the first UI commit with English (`en`) and Arabic (`ar`) resources plus LTR/RTL direction switching. No later task may introduce untranslated user-visible strings.
- The TASK-00 Pull Request itself must exercise the newly added CI workflow; Codex must not merge it.
...
- Open the React application and verify the system-status/home screen can reach the API.
- Switch the UI from English to Arabic and verify direction changes to RTL; switch back to English and verify LTR.
- Refresh a routed page and verify the application still loads correctly.
- Open API/OpenAPI and health/readiness URLs reported by Codex and verify they respond successfully.
...
```bash
git switch -c task/00-bootstrap-runtime
```
...
git status
git add .
git commit -m "feat: complete TASK-00 bootstrap-runtime"
git push -u origin task/00-bootstrap-runtime
```

# TASK-08 — Unit Approval Workflow

    ## Objective

    Implement the Unit Officer approval/rejection step as a complete audited workflow slice.
...
- `docs/loan-system-api-boundary.md`
    ...
    - Implement Unit approve/reject domain behaviors and command handlers.
- Enforce Submitted-state prerequisite and `loanApplications.unitApprove` permission.
- Persist actor, timestamp, decision, comment/reason.
- Emit domain/audit-relevant events without directly writing Audit module tables.

    ## Frontend Scope

    - Create Unit Review queue/filter for Submitted applications.
- Create application review view with Approve/Reject controls and required rejection reason.
- Show decision history.

    ## Persistence / Infrastructure Scope

    - Add migration if approval representation requires new normalized structure.

    ## Required Tests

    - Domain state-transition tests.
- Integration tests for approve/reject, wrong state, missing reason, permission denial, stale concurrency.
- Frontend review/action tests.
- E2E smoke: submitted application → unit approval.

    ## Acceptance Criteria

    - [ ] Only authorized Unit approval action can transition the application.
- [ ] Rejected application cannot proceed to Committee step.
- [ ] Decision metadata is persisted and visible.
- [ ] All quality and coverage gates pass.

## Scope / Documentation Alignment Requirements

- Preserve source queue semantics by providing a Submitted review queue for Unit Officers. Advanced waiting-list prioritization is not invented without rules.
...
- Login as Unit Officer, open the Submitted queue, review an application and approve it.
- Reject another submitted application with a required reason and verify the decision history.
- Use a user without Unit approval permission and verify approval cannot be performed.
- Try to approve an application in the wrong state and verify it is rejected.
...
```bash
git switch -c task/08-unit-approval
```
...
git status
git add .
git commit -m "feat: complete TASK-08 unit-approval"
git push -u origin task/08-unit-approval
```

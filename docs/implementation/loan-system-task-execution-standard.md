# Loan Management System — Task Execution Standard
...
```bash
git switch -c task/<task-number>-<task-slug>
```
...
git status
git add .
git commit -m "feat: complete TASK-XX <task-name>"
git push -u origin task/<task-number>-<task-slug>
```

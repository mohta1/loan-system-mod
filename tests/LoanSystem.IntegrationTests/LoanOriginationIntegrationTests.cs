...
public sealed class LoanOriginationIntegrationTests(IdentitySqlFixture fixture)
{
    ...
    [Fact]
    public async Task Task10_readiness_documents_mortgage_and_inspection_concurrency_use_real_http_and_sql()
    {
        using var admin = await Administrator();
        var setup = await SetupAvailableVersion(admin);
        var created = await admin.PostAsJsonAsync("/api/v1/loan-applications", ApplicationInput(setup));
        var application = await Read(created); var id = application.GetProperty("loanApplicationId").GetGuid();
        var evaluated = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/evaluate-eligibility", created.Headers.ETag!.Tag.Trim('"'), new { });
        var submitted = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/submit", evaluated.Headers.ETag!.Tag.Trim('"'), new { });
        var unit = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/unit-decision", submitted.Headers.ETag!.Tag.Trim('"'), new { decision = "approve" });
        var committee = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/committee-decision", unit.Headers.ETag!.Tag.Trim('"'), new { decision = "approve" });
        var noPrecondition = await admin.PostAsJsonAsync($"/api/v1/loan-applications/{id}/inspections", new { }); Assert.Equal((HttpStatusCode)428, noPrecondition.StatusCode);
        var inspectionCreated = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/inspections", committee.Headers.ETag!.Tag.Trim('"'), new { });
        Assert.Equal(HttpStatusCode.Created, inspectionCreated.StatusCode); var inspection = await Read(inspectionCreated); var inspectionId = inspection.GetProperty("propertyInspectionId").GetGuid(); var draftTag = inspectionCreated.Headers.ETag!.Tag.Trim('"');
        var pending = await Read(await admin.GetAsync($"/api/v1/loan-applications/{id}")); Assert.Equal("PrerequisitesPending", pending.GetProperty("status").GetString()); Assert.Equal("Pending", pending.GetProperty("inspectionPrerequisiteStatus").GetString());
        var edited = await Send(admin, HttpMethod.Put, $"/api/v1/inspections/{inspectionId}", draftTag, new { governorate = "Muscat", state = "Bawshar", area = "Khuwair", numberOfFloors = 2, numberOfRooms = 4, propertyArea = 250m, propertyCondition = "Good", inspectionDate = "2026-09-13", result = "Suitable", notes = "Recorded" });
        Assert.Equal(HttpStatusCode.OK, edited.StatusCode); var editedTag = edited.Headers.ETag!.Tag.Trim('"');
        var completed = await Send(admin, HttpMethod.Post, $"/api/v1/inspections/{inspectionId}/complete", editedTag, new { }); Assert.Equal(HttpStatusCode.OK, completed.StatusCode); var recordedTag = completed.Headers.ETag!.Tag.Trim('"');
        var approved = await Send(admin, HttpMethod.Post, $"/api/v1/inspections/{inspectionId}/decision", recordedTag, new { decision = "approve" }); Assert.Equal(HttpStatusCode.OK, approved.StatusCode);
        var stale = await Send(admin, HttpMethod.Put, $"/api/v1/inspections/{inspectionId}", recordedTag, new { governorate = "x", state = "x", area = "x", numberOfFloors = 1, numberOfRooms = 0, propertyArea = 1, propertyCondition = "x", inspectionDate = "2026-09-13", result = "x" }); Assert.Equal(HttpStatusCode.PreconditionFailed, stale.StatusCode); Assert.Equal("inspections.concurrencyConflict", (await Read(stale)).GetProperty("errorCode").GetString());
        async Task<Guid> Upload(string name) { using var form = new MultipartFormDataContent(); form.Add(new ByteArrayContent(System.Text.Encoding.UTF8.GetBytes("task10")) { Headers = { ContentType = new("text/plain") } }, "file", name); var response = await admin.PostAsync("/api/v1/documents", form); Assert.Equal(HttpStatusCode.Created, response.StatusCode); return (await Read(response)).GetProperty("documentId").GetGuid(); }
        var appResponse = await admin.GetAsync($"/api/v1/loan-applications/{id}");
        Guid detached = Guid.Empty;
        foreach (var type in new[] { "Ownership", "Survey", "EngineeringDrawing" }) { var documentId = await Upload($"{type}-{Guid.NewGuid():N}.txt"); detached = documentId; var attached = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/documents", appResponse.Headers.ETag!.Tag.Trim('"'), new { documentId, documentType = type }); Assert.Equal(HttpStatusCode.OK, attached.StatusCode); appResponse = attached; }
        Assert.Equal("Satisfied", (await Read(appResponse)).GetProperty("documentPrerequisiteStatus").GetString());
        var mortgage = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/mortgage/completed", appResponse.Headers.ETag!.Tag.Trim('"'), new { }); Assert.Equal(HttpStatusCode.OK, mortgage.StatusCode); var ready = await Read(mortgage); Assert.Equal("ReadyForFinalApproval", ready.GetProperty("status").GetString());
        var repeat = await Send(admin, HttpMethod.Post, $"/api/v1/loan-applications/{id}/mortgage/completed", mortgage.Headers.ETag!.Tag.Trim('"'), new { }); Assert.Equal(HttpStatusCode.Conflict, repeat.StatusCode); Assert.Equal("loanApplications.mortgageAlreadyDecided", (await Read(repeat)).GetProperty("errorCode").GetString());
        var detachedResponse = await Send(admin, HttpMethod.Delete, $"/api/v1/loan-applications/{id}/documents/{detached}", mortgage.Headers.ETag!.Tag.Trim('"'), new { }); var detachedBody = await Read(detachedResponse); Assert.Equal("Pending", detachedBody.GetProperty("documentPrerequisiteStatus").GetString()); Assert.Equal("PrerequisitesPending", detachedBody.GetProperty("status").GetString());
        using var scope = fixture.Factory.Services.CreateScope(); var db = scope.ServiceProvider.GetRequiredService<LoanOriginationDbContext>(); var storedInspection = await db.PropertyInspections.AsNoTracking().SingleAsync(x => x.Id == inspectionId); Assert.Equal(PropertyInspectionStatus.Approved, storedInspection.Status); Assert.NotNull(storedInspection.FinalizedAtUtc); var stored = await db.LoanApplications.AsNoTracking().SingleAsync(x => x.Id == id); Assert.Equal(MortgageStatus.Completed, stored.MortgageStatus); Assert.NotNull(stored.MortgageDecision);
    }

...
public sealed class DocumentsIntegrationTests(IdentitySqlFixture fixture)
{
    ...
    public async Task Real_sql_upload_download_delete_and_authorization()
    {
        ...
        Assert.Equal(HttpStatusCode.NoContent, (await owner.DeleteAsync($"/api/v1/documents/{id}")).StatusCode); Assert.Equal(HttpStatusCode.NotFound, (await owner.GetAsync($"/api/v1/documents/{id}")).StatusCode); using var deletedScope = fixture.Factory.Services.CreateScope(); var persisted = await deletedScope.ServiceProvider.GetRequiredService<DocumentsDbContext>().Documents.SingleAsync(x => x.Id == id); Assert.Equal(LoanSystem.Modules.Documents.Domain.DocumentStatus.Deleted, persisted.Status);
    }

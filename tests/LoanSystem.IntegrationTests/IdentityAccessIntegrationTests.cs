...
public sealed class IdentityAccessIntegrationTests(IdentitySqlFixture fixture)
{
    ...
    public async Task Clean_migrations_and_seed_match_authorization_matrix()
    {
        ...
        Assert.Equal(11, await db.Roles.CountAsync()); Assert.Equal(41, await db.Permissions.CountAsync());
        var unit = await db.Roles.Include(x => x.RolePermissions).ThenInclude(x => x.Permission).SingleAsync(x => x.Name == "Unit Officer"); Assert.Contains(unit.RolePermissions, x => x.Permission.Key == "loanApplications.unitApprove"); Assert.DoesNotContain(unit.RolePermissions, x => x.Permission.Key == "identity.users.manage"); var committee = await db.Roles.Include(x => x.RolePermissions).ThenInclude(x => x.Permission).SingleAsync(x => x.Name == "Loan Committee Member"); Assert.Contains(committee.RolePermissions, x => x.Permission.Key == "loanApplications.read"); Assert.Contains(committee.RolePermissions, x => x.Permission.Key == "loanApplications.committeeApprove"); Assert.DoesNotContain(committee.RolePermissions, x => x.Permission.Key == "loanApplications.unitApprove"); Assert.DoesNotContain(committee.RolePermissions, x => x.Permission.Key == "loanApplications.create"); Assert.DoesNotContain(committee.RolePermissions, x => x.Permission.Key == "loanApplications.update"); Assert.DoesNotContain(committee.RolePermissions, x => x.Permission.Key == "loanApplications.submit");
    ...
    public async Task Login_me_logout_and_generic_failures_are_secure()
    {
        ...
        var users = JsonDocument.Parse(await (await client.GetAsync("/api/v1/users")).Content.ReadAsStringAsync()).RootElement; var self = users.EnumerateArray().Single(x => x.GetProperty("username").GetString() == Admin); var selfId = self.GetProperty("userId").GetGuid(); var selfTag = self.GetProperty("eTag").GetString()!;
        Assert.Equal(HttpStatusCode.Conflict, (await Send(client, HttpMethod.Post, $"/api/v1/users/{selfId}/deactivate", selfTag, new { })).StatusCode);
        Assert.Equal(HttpStatusCode.Conflict, (await Send(client, HttpMethod.Put, $"/api/v1/users/{selfId}/roles", selfTag, new { roleIds = Array.Empty<Guid>() })).StatusCode);

...
public sealed class IdentityAccessIntegrationTests : IAsyncLifetime
{
    private MsSqlContainer? sql;
    private WebApplicationFactory<Program>? factory;
    private const string Admin = "integration-admin"; private const string Password = "Disposable_integration_password_123!";
    public async Task InitializeAsync()
    {
        if (!DockerAvailable()) return; sql = new MsSqlBuilder("mcr.microsoft.com/mssql/server:2022-latest").Build(); await sql.StartAsync();
        factory = new WebApplicationFactory<Program>().WithWebHostBuilder(builder => { builder.UseEnvironment("Development"); builder.ConfigureAppConfiguration((_, config) => config.AddInMemoryCollection(new Dictionary<string, string?> { ["ConnectionStrings:LoanSystem"] = sql!.GetConnectionString(), ["Database:AutoMigrate"] = "true", ["DevelopmentAdmin:Username"] = Admin, ["DevelopmentAdmin:Password"] = Password, ["DevelopmentAdmin:DisplayName"] = "Integration Administrator" })); });
        using var client = factory.CreateClient(); Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/health/live")).StatusCode);
    }
    public async Task DisposeAsync() { if (factory is not null) await factory.DisposeAsync(); if (sql is not null) await sql.DisposeAsync(); }
    [Fact]
    public async Task Clean_migrations_and_seed_match_authorization_matrix()
    {
        if (factory is null) return; await using var connection = new SqlConnection(sql!.GetConnectionString()); await connection.OpenAsync();
        ...
        using var scope = factory.Services.CreateScope(); var db = scope.ServiceProvider.GetRequiredService<IdentityAccessDbContext>(); Assert.Equal(11, await db.Roles.CountAsync()); Assert.Equal(40, await db.Permissions.CountAsync());
        ...
        await factory.Services.InitializeIdentityAccessAsync(new ConfigurationBuilder().Build(), false); Assert.Equal(11, await db.Roles.CountAsync());
    ...
    {
        if (factory is null) return; using var client = factory.CreateClient(new WebApplicationFactoryClientOptions { HandleCookies = true });
    ...
    {
        if (factory is null) return; using var admin = factory.CreateClient(new WebApplicationFactoryClientOptions { HandleCookies = true }); await Login(admin); Assert.Equal(HttpStatusCode.OK, (await admin.GetAsync("/api/v1/users")).StatusCode);
        ...
        using var limited = factory.CreateClient(new WebApplicationFactoryClientOptions { HandleCookies = true }); await Login(limited, "limited-user", "Disposable_user_password_123!"); Assert.Equal(HttpStatusCode.Forbidden, (await limited.GetAsync("/api/v1/users")).StatusCode);
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, $"/api/v1/users/{id}/deactivate"); request.Headers.TryAddWithoutValidation("If-Match", $"\"{etag}\""); var disabled = await admin.SendAsync(request); Assert.Equal(HttpStatusCode.OK, disabled.StatusCode); Assert.Equal(HttpStatusCode.Unauthorized, (await limited.GetAsync("/api/v1/auth/me")).StatusCode); Assert.Equal(HttpStatusCode.Unauthorized, (await factory.CreateClient().PostAsJsonAsync("/api/v1/auth/login", new { username = "limited-user", password = "Disposable_user_password_123!" })).StatusCode);
    }
    [Fact]
    public async Task Multiple_roles_and_all_stale_mutations_return_precondition_failed()
    {
        if (factory is null) return; using var client = factory.CreateClient(new WebApplicationFactoryClientOptions { HandleCookies = true }); await Login(client); var created = await client.PostAsJsonAsync("/api/v1/users", new { username = "concurrency-user", displayName = "Concurrency", password = "Disposable_user_password_123!" }); var user = JsonDocument.Parse(await created.Content.ReadAsStringAsync()).RootElement; var id = user.GetProperty("userId").GetGuid(); var stale = user.GetProperty("eTag").GetString()!;

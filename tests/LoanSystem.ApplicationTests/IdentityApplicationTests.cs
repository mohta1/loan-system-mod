...
public sealed class IdentityApplicationTests
{
    ...
    [Fact] public void Identity_contract_dtos_expose_values() { var id = Guid.NewGuid(); var role = Guid.NewGuid(); var user = new UserDto(id, "u", "User", true, [role], "etag"); var roleDto = new RoleDto(role, "Role"); var current = new CurrentUserDto(id, "u", "User", ["Role"], [IdentityPermissions.ManageUsers]); var command = new CreateUserCommand("u", "User", "password-1234"); Assert.Equal(id, user.UserId); Assert.True(user.IsActive); Assert.Equal("etag", user.ETag); Assert.Equal("Role", roleDto.Name); Assert.Contains("Role", current.Roles); Assert.Contains(IdentityPermissions.ManageUsers, current.Permissions); Assert.Equal("password-1234", command.Password); }
    ...
    private sealed class Store : IIdentityStore
    {
        ...
        public Task<bool> AreRolesValidAsync(IReadOnlyCollection<Guid> ids, CancellationToken ct) => Task.FromResult(true);
